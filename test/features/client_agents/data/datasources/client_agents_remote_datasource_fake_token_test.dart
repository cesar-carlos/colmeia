...
void main() {
  ...
  test(
    'fetchClientAgentToken returns null when no token has been stored',
    () async {
  ...
  test(
    'fetchClientAgentToken throws DioException 403 when no access',
    () async {
  ...
  test(
    'putClientAgentToken stores token and round-trips on subsequent GET',
    () async {
  ...
  test(
    ...
    () async {
      final before = await fake.fetchApprovedAgents(
      ...
      final after = await fake.fetchApprovedAgents(
        query: const PaginatedQuery(),
      );
