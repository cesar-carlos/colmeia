  ...
  group('CachingResumoTotalDiarioVendasRepositoryImpl', () {
    ...
    test(
      ...
      () async {
        ...
        fakeDelegate.rows = <ResumoTotalDiarioVendasRow>[
          ResumoTotalDiarioVendasRow(
            codEmpresa: 1,
            codFilial: 1,
            dataVenda: DateTime(2026, 6, 3),
            qtdVendas: 3,
            valorTotalDiarioVenda: 3,
          ),
          ResumoTotalDiarioVendasRow(
            codEmpresa: 1,
            codFilial: 1,
            dataVenda: DateTime(2026, 6, 4),
            qtdVendas: 4,
            valorTotalDiarioVenda: 4,
          ),
          ResumoTotalDiarioVendasRow(
            codEmpresa: 1,
            codFilial: 1,
            dataVenda: DateTime(2026, 6, 5),
            qtdVendas: 5,
            valorTotalDiarioVenda: 5,
          ),
        ];
        ...
        expect(fakeDelegate.loadCount, 1);
      },
    );

    test('forceRefresh merges bucket rows in plan order', () async {
      cachingRepo = CachingResumoTotalDiarioVendasRepositoryImpl(
        delegate: _RangeAwareDelegate(),
        factsStore: memoryAgentQueryFactsStore(),
        clock: () => clock,
        useExecuteBatchForBuckets: false,
      );
    ...
    test('returns range coalesce failure from delegate', () async {
      cachingRepo = CachingResumoTotalDiarioVendasRepositoryImpl(
        delegate: _AlwaysFailingDelegate(),
        factsStore: memoryAgentQueryFactsStore(),
        clock: () => clock,
        useExecuteBatchForBuckets: false,
      );
      ...
      expect(result.exceptionOrNull()?.message, 'range-failed');
    });
  });
}
...
final class _RangeAwareDelegate implements ResumoTotalDiarioVendasRepository {
  @override
  Future<AppResult<List<ResumoTotalDiarioVendasRow>>> load({
    required String userId,
    required String agentId,
...
final class _AlwaysFailingDelegate
  ...
  Future<AppResult<List<ResumoTotalDiarioVendasRow>>> load({
    ...
    return const Failure<List<ResumoTotalDiarioVendasRow>, AppFailure>(
      ValidationFailure(message: 'range-failed'),
    );
  }
}
