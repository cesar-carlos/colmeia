@Tags(['e2e'])
library;

import 'package:colmeia/core/config/app_environment.dart';
import 'package:colmeia/core/di/injector.dart';
import 'package:colmeia/features/agent_queries/application/usecases/load_grupo_marca_produto_options_use_case.dart';
import 'package:colmeia/features/agent_queries/domain/repositories/grupo_marca_produto_options_repository.dart';
import 'package:flutter_test/flutter_test.dart' hide group;
import 'package:test_api/scaffolding.dart' show group;

import 'support/e2e_agent_queries_test_helpers.dart';

void main() {
  group(
    'GrupoMarcaProdutoOptionsRepository (e2e)',
    () {
      registerE2eAgentQueriesSuiteHooks();

      test(
        'loadGrupoAndMarcaOptions executes the real batch query',
        () async {
          if (shouldSkipE2eRepositoryTest(
            'grupo_marca_produto_options_repository_e2e',
          )) {
            return;
          }

          final repository = getIt<GrupoMarcaProdutoOptionsRepository>();

          final result = await runE2eAppResult(
            () => repository.loadGrupoAndMarcaOptions(
              userId: 'user-1',
              agentId: AppEnvironment.e2eAgentId,
              clientToken: AppEnvironment.e2eClientToken,
            ),
          );

          result.fold(
            (batch) {
              expect(batch.grupoOptions.length, lessThanOrEqualTo(20));
              expect(batch.marcaOptions.length, lessThanOrEqualTo(20));
              for (final row in batch.grupoOptions) {
                expect(row.codGrupoProduto, greaterThan(0));
                expect(row.nomeGrupoProduto, isNotEmpty);
              }
              for (final row in batch.marcaOptions) {
                expect(row.codMarca, greaterThan(0));
                expect(row.nomeMarca, isNotEmpty);
              }
            },
            (failure) {
              expectAcceptableAgentQueriesE2eFailure(
                failure,
                failureScope: 'Repository e2e',
              );
            },
          );
        },
      );

      test(
        'use case executes the same grupo and marca batch query',
        () async {
          if (shouldSkipE2eRepositoryTest(
            'load_grupo_marca_produto_options use_case e2e',
          )) {
            return;
          }

          final useCase = getIt<LoadGrupoMarcaProdutoOptionsUseCase>();
          final result = await runE2eAppResult(
            () => useCase(
              userId: 'user-1',
              agentId: AppEnvironment.e2eAgentId,
              page: 2,
              clientToken: AppEnvironment.e2eClientToken,
            ),
          );

          result.fold(
            (batch) {
              expect(batch.grupoOptions.length, lessThanOrEqualTo(20));
              expect(batch.marcaOptions.length, lessThanOrEqualTo(20));
            },
            (failure) {
              expectAcceptableAgentQueriesE2eFailure(
                failure,
                failureScope: 'Use-case e2e',
              );
            },
          );
        },
      );
    },
    tags: <String>['e2e'],
  );
}
