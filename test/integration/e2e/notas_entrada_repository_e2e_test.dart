...
void main() {
  group(
    ...
    () {
      ...
      test(
        'loadPage recuts the numbered catalog by CodFornecedor equality',
        () async {
          if (shouldSkipE2eRepositoryTest('notas_entrada_repository_e2e')) {
            return;
          }
          ...
          unfiltered.fold(
            ...
            (failure) => expectAcceptableAgentQueriesE2eFailure(
              failure,
              failureScope: 'Supplier code baseline repository e2e',
            ),
          );
          final baseline = catalog;
          if (baseline == null || baseline.items.isEmpty) {
          ...
          final expectedCode = baseline.items.first.codFornecedor;
          final scoped = await repository.loadPage(
            userId: 'user-1',
            agentId: AppEnvironment.e2eAgentId,
            clientToken: AppEnvironment.e2eClientToken,
            filter: NotasEntradaFilter(
              pageSize: 25,
              codFornecedor: expectedCode,
            ),
          );
          scoped.fold(
            (page) {
            ...
            (failure) => expectAcceptableAgentQueriesE2eFailure(
              failure,
              failureScope: 'Supplier code repository e2e',
            ),
          );
        },
      );
