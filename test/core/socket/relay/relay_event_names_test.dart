...
void main() {
  group('RelayEventNames', () {
    test('keeps the wire strings expected by plug_server docs', () {
      ...
      ).equals('relay:conversation.start');
      check(
      ...
      check(
        RelayEventNames.rpcStreamPullResponse,
  ...
  group('RelayPayloadFrameCompression', () {
    test('parse maps wire strings to the canonical enum', () {
      check(
        RelayPayloadFrameCompression.parse('always'),
      ).equals(RelayPayloadFrameCompression.always);
      check(
        RelayPayloadFrameCompression.parse('  ALWAYS  '),
      ).equals(RelayPayloadFrameCompression.always);
      check(
        RelayPayloadFrameCompression.parse('none'),
      ).equals(RelayPayloadFrameCompression.none);
      check(
        RelayPayloadFrameCompression.parse('default'),
      ).equals(RelayPayloadFrameCompression.auto);
      check(
        RelayPayloadFrameCompression.parse(null),
      ).equals(RelayPayloadFrameCompression.auto);
      check(
        RelayPayloadFrameCompression.parse('something else'),
      ).equals(RelayPayloadFrameCompression.auto);
    });
