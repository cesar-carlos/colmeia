...
void main() {
  group('ConnectionReadyCompatMode.parse', () {
    ...
    test('parses payload_frame_only and aliases', () {
      check(
        ConnectionReadyCompatMode.parse('payload_frame_only'),
      ).equals(ConnectionReadyCompatMode.payloadFrameOnly);
      check(
        ConnectionReadyCompatMode.parse('payloadFrameOnly'),
      ).equals(ConnectionReadyCompatMode.payloadFrameOnly);
      check(
        ConnectionReadyCompatMode.parse('payload-frame'),
      ).equals(ConnectionReadyCompatMode.payloadFrameOnly);
    });

    test('parses raw_json_only and aliases', () {
      check(
        ConnectionReadyCompatMode.parse('raw_json_only'),
      ).equals(ConnectionReadyCompatMode.rawJsonOnly);
      check(
        ConnectionReadyCompatMode.parse('rawJsonOnly'),
      ).equals(ConnectionReadyCompatMode.rawJsonOnly);
      check(
        ConnectionReadyCompatMode.parse('raw-json'),
      ).equals(ConnectionReadyCompatMode.rawJsonOnly);
    });
