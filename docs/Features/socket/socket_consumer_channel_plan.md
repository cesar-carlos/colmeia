...
> 2. **Relay mode selector**: entregue. `useRelay` escolhe relay;
>    `relayMode` escolhe unary (default) ou streaming por chamada.
...
| **Relay mode selector** — `useRelay` escolhe transporte relay, enquanto `AgentSqlExecuteRequest.relayMode` escolhe unary (default) ou streaming. O DI usa roteador relay: `sql.execute` unary por padrão, streaming só com `relayMode: streaming`, e `sql.executeBatch` sempre unary. | `AgentSqlExecuteRequest` + `RoutingRelayAgentQueriesRemoteDataSource` + `injector_agent_queries.dart` | pós-alinhamento |
...
- **Relay mode selector (entregue)** — `useRelay: true` escolhe o
  transporte relay. `AgentSqlExecuteRequest.relayMode` escolhe unary
  (default) ou streaming; `sql.executeBatch` permanece sempre relay
  unary. Streaming progressivo e opt-in por chamada, nao um default do
  relay.
...
| **Relay mode selector (entregue)** | `useRelay` escolhe transporte relay; `AgentSqlExecuteRequest.relayMode` escolhe unary (default) ou streaming. O roteador relay usa unary para `sql.execute` por padrao, streaming apenas com `relayMode: streaming`, e `sql.executeBatch` sempre unary.                                                                                                                     | 2       |
