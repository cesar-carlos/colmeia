...
Current rollout policy: Colmeia defaults `api_version` to **`2.10`**. Large report/chart queries use `useRelay: true`, `relayMode: streaming`, and `options.prefer_db_streaming: true`; lookup/options queries remain relay unary. **Exceptions:** product billing ranking, product profit ranking, monthly product profitability (`ResumoProdutoVendaLucratividadeMensal`), period product profitability (`ResumoProdutoVendaLucratividade`), paged product sales (`ResumoProdutoVenda`), product sales trend page/summary (`ProdutoVendidoTendenciaDeVenda`), moving-average trend page/summary (`ProdutoVendidoTendenciaDeVendaMediaMovel`), daily sales totals (`ResumoTotalDiarioVendas`), and live-map period aggregates (`ResumoTotalVendasMunicipioFilialPeriodo`) stay relay unary without DB streaming until the SQL Anywhere empty-payload issue is fixed — see the canonical note in [`../../bridge_agent_sql_api_options.md`](../../bridge_agent_sql_api_options.md). Overview read-only `sql.executeBatch` calls set `options.max_parallel_read_only_batch_items: 4`. Do not use `meta.outbound_compression` as the main performance tuning knob; the current server runtime documents it as a no-op.
